...
def forward_kinematics(skeleton, cur_joint_idx, local_pose, global_pose):
    ...
    for joint_id in child_id:
        global_pose[joint_id] = torch.matmul(
            global_pose[cur_joint_idx], local_pose[joint_id]
        )
...
def euler2mat(theta, to_4x4=False):
    ...
    R_x = torch.cat(
        [
            ...
            torch.stack(
                [torch.zeros_like(theta_x), torch.cos(theta_x), -torch.sin(theta_x)], 2
            ),
            torch.stack(
                [torch.zeros_like(theta_x), torch.sin(theta_x), torch.cos(theta_x)], 2
            ),
        ],
        1,
    )
    ...
    R_y = torch.cat(
        [
            torch.stack(
                [torch.cos(theta_y), torch.zeros_like(theta_y), torch.sin(theta_y)], 2
            ),
            torch.stack(
                [
                    torch.zeros_like(theta_y),
                    torch.ones_like(theta_y),
                    torch.zeros_like(theta_y),
                ],
                2,
            ),
            torch.stack(
                [-torch.sin(theta_y), torch.zeros_like(theta_y), torch.cos(theta_y)], 2
            ),
        ],
        1,
    )
    ...
    R_z = torch.cat(
        [
            torch.stack(
                [torch.cos(theta_z), -torch.sin(theta_z), torch.zeros_like(theta_z)], 2
            ),
            torch.stack(
                [torch.sin(theta_z), torch.cos(theta_z), torch.zeros_like(theta_z)], 2
            ),
            torch.stack(
                [
                    torch.zeros_like(theta_z),
                    torch.zeros_like(theta_z),
                    torch.ones_like(theta_z),
                ],
                2,
            ),
        ],
    ...
    if to_4x4:
        ...
        R = torch.cat(
            [
                R,
                torch.cuda.FloatTensor([0, 0, 0, 1])[None, None, :].repeat(
                    batch_size, 1, 1
                ),
            ],
            1,
