...
def vis_2d_keypoints(img, kps, kps_line, bbox=None, kp_thre=0.4, alpha=1):
    ...
    if bbox is not None:
        ...
        cv2.line(
            kp_mask, b1, b2, color=(255, 255, 0), thickness=1, lineType=cv2.LINE_AA
        )
        cv2.line(
            kp_mask, b2, b3, color=(255, 255, 0), thickness=1, lineType=cv2.LINE_AA
        )
        cv2.line(
            kp_mask, b3, b4, color=(255, 255, 0), thickness=1, lineType=cv2.LINE_AA
        )
        cv2.line(
            kp_mask, b4, b1, color=(255, 255, 0), thickness=1, lineType=cv2.LINE_AA
        )
...
def vis_3d_pose(
    kps_3d, kps_line, joint_set_name="", prefix="vis3dpose", gt=False, ax_in=None
):
    ...
    if not ax_in:
        ...
        now = datetime.now()
        file_name = (
            f'{prefix}_{now.isoformat()[:-7]}_{"3d_gt" if gt else "3d_pred"}.jpg'
        )
