...
class MGCN(nn.Module):
    ...
    def __init__(
        self, in_features, out_features, in_channel, adj, bias=True, fix=False
    ):
        ...
        if adj == None:
            adj = nn.Parameter(
                torch.eye(in_channel, in_channel, dtype=torch.float, requires_grad=True)
            )
        ...
        if fix:
            self.adj2 = nn.Parameter(torch.zeros_like(adj), requires_grad=False)
        else:
...
class Block(nn.Module):
    def __init__(
        ...
        self.mgcn = MGCN(dim, dim, in_channel, graph_adj, fix=fix)
        self.attn = Attention(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
        ...
        self.mlpse = MLP_SE(
            in_features=dim, in_channel=in_channel, hidden_features=mlp_hidden_dim
        )
