...
def prepare_network(args, load_dir="", is_train=True):
    ...
    if is_train or load_dir:
        ...
        if cfg.MODEL.name == "GTRS_net":
            model = models.GTRS_net.get_model(
                num_joint=main_dataset.joint_num, graph_adj=main_dataset.graph_Adj
            )
        elif cfg.MODEL.name == "PAM":
            model = models.PAM.get_model(
                main_dataset.joint_num,
                embed_dim=128,
                graph_adj=main_dataset.graph_Adj,
    ...
    if load_dir and (not is_train or args.resume_training):
        ...
        checkpoint = load_checkpoint(
            load_dir=load_dir, pick_best=(cfg.MODEL.name == "PAM")
        )
        ...
        if is_train:
            ...
            # update lr_scheduler
            lr_state["milestones"], lr_state["gamma"] = (
                Counter(cfg.TRAIN.lr_step),
                cfg.TRAIN.lr_factor,
            )
            ...
            print(
                "===> resume from epoch {:d}, current lr: {:.0e}, milestones: {}, lr factor: {:.0e}".format(
                    cfg.TRAIN.begin_epoch,
                    curr_lr,
                    lr_state["milestones"],
                    lr_state["gamma"],
                )
...
class Trainer:
    def __init__(self, args, load_dir):
        ...
        self.J_regressor = eval(
            f"torch.Tensor(self.main_dataset.joint_regressor_{cfg.DATASET.target_joint_set}).cuda()"
        )
    ...
    def train(self, epoch):
        ...
        for i, (inputs, targets, meta) in enumerate(batch_generator):
            ...
            gt_lift3dpose, gt_reg3dpose, gt_mesh = (
                targets["lift_pose3d"].cuda(),
                targets["reg_pose3d"].cuda(),
                targets["mesh"].cuda(),
            )
            val_lift3dpose, val_reg3dpose, val_mesh = (
                meta["lift_pose3d_valid"].cuda(),
                meta["reg_pose3d_valid"].cuda(),
                meta["mesh_valid"].cuda(),
            )
            ...
            # loss
            loss1, loss2, loss4, loss5 = (
                self.loss[0](pred_mesh, gt_mesh, val_mesh),
                self.normal_weight * self.loss[1](pred_mesh, gt_mesh),
                self.joint_weight
                * self.loss[3](pred_pose, gt_reg3dpose, val_reg3dpose),
                self.joint_weight
                * self.loss[4](lift_pose, gt_lift3dpose, val_lift3dpose),
            )
            loss3 = 0
...
class Tester:
    def __init__(self, args, load_dir=""):
        self.val_loader, self.val_dataset, self.model, _, _, _, _, _ = prepare_network(
            args, load_dir=load_dir, is_train=False
        )
        ...
        self.J_regressor = eval(
            f"torch.Tensor(self.val_dataset.joint_regressor_{cfg.DATASET.target_joint_set}).cuda()"
        )
    ...
    def test(self, epoch, current_model=None):
        ...
        with torch.no_grad():
            for i, (inputs, targets, meta) in enumerate(loader):
                ...
                j_error, s_error = self.val_dataset.compute_both_err(
                    pred_mesh, gt_mesh, pred_pose, gt_pose3d
                )
                ...
                if i % self.print_freq == 0:
                    loader.set_description(
                        f"{eval_prefix}({i}/{len(self.val_loader)}) => surface error: {s_error:.4f}, joint error: {j_error:.4f}"
                    )
                ...
                if epoch == 0 or epoch == cfg.TRAIN.end_epoch:
                    pred_mesh, target_mesh = (
                        ...
                        gt_mesh.detach().cpu().numpy(),
                    )
                    for j in range(len(input_pose)):
                        out = {}
                        out["mesh_coord"], out["mesh_coord_target"] = (
                            pred_mesh[j],
                            target_mesh[j],
                        )
                        result.append(out)
            ...
            print(
                f"{eval_prefix} MPVPE: {self.surface_error:.2f}, MPJPE: {self.joint_error:.2f}"
            )
    ...
    def test(self, epoch, current_model=None):
        ...
        with torch.no_grad():
            for i, (img_joint, cam_joint, _) in enumerate(loader):
                img_joint, cam_joint = (
                    img_joint.cuda().float(),
                    cam_joint.cuda().float(),
                )
                ...
                if i % self.print_freq == 0:
                    loader.set_description(
                        f"{eval_prefix}({i}/{len(self.val_loader)}) => joint error: {mpjpe:.4f}"
                    )
                ...
                if epoch == 0 or epoch == cfg.TRAIN.end_epoch:
                    pred_joint, target_joint = (
                        ...
                        cam_joint.detach().cpu().numpy(),
                    )
                    for j in range(len(pred_joint)):
                        out = {}
                        out["joint_coord"], out["joint_coord_target"] = (
                            pred_joint[j],
                            target_joint[j],
                        )
                        result.append(out)
