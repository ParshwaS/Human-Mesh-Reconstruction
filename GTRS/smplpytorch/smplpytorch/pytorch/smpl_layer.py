...
from smplpytorch.pytorch.tensutils import (
    th_posemap_axisang,
    th_with_zeros,
    th_pack,
    make_list,
...
class SMPL_Layer(Module):
    ...
    def __init__(
        self, center_idx=None, gender="neutral", model_root="smpl/native/models"
    ):
        """
        Args:
        ...
        if gender == "neutral":
            self.model_path = os.path.join(
                model_root, "basicModel_neutral_lbs_10_207_0_v1.0.0.pkl"
            )
        elif gender == "female":
            self.model_path = os.path.join(
                model_root, "basicModel_f_lbs_10_207_0_v1.0.0.pkl"
            )
        elif gender == "male":
            self.model_path = os.path.join(
                model_root, "basicModel_m_lbs_10_207_0_v1.0.0.pkl"
            )
    ...
    def forward(
        self, th_pose_axisang, th_betas=torch.zeros(1), th_trans=torch.zeros(1)
    ):
        """
        Args:
        ...
        for i in range(self.num_joints - 1):
            i_val = int(i + 1)
            joint_rot = (
                th_pose_rotmat[:, (i_val - 1) * 9 : i_val * 9]
                .contiguous()
                .view(batch_size, 3, 3)
            )
        ...
        th_results2 = torch.zeros(
            (batch_size, 4, 4, self.num_joints),
            dtype=root_j.dtype,
        ...
        for i in range(self.num_joints):
            ...
            joint_j = torch.cat(
                [th_j[:, i], padd_zero.view(1, 1).repeat(batch_size, 1)], 1
            )
