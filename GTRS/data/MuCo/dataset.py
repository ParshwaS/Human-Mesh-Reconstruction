...
class MuCo(torch.utils.data.Dataset):
    def __init__(self, data_split, args):
        ...
        self.smpl_param_path = osp.join(
            cfg.data_dir, dataset_name, "data", "smpl_param.json"
        )
    ...
    def load_data(self):
        ...
        for iid in db.imgs.keys():
            ...
            root_depths = [
                ann["keypoints_cam"][self.muco_root_joint_idx][2] for ann in anns
            ]
            ...
            for pid in pid_list:
                ...
                try:
                    ...
                    pose, shape, trans = (
                        np.array(smpl_param["pose"]),
                        np.array(smpl_param["shape"]),
                        np.array(smpl_param["trans"]),
                    )
    ...
    def get_joints_from_mesh(self, mesh, joint_set_name, cam_param):
        ...
        # projection
        f, c = (
            np.array(cam_param["focal"], dtype=np.float32),
            np.array(cam_param["princpt"], dtype=np.float32),
        )
    ...
    def get_fitting_error(self, muco_joint, smpl_mesh):
        ...
        muco_joint = (
            muco_joint - muco_joint[self.muco_root_joint_idx, None, :]
        )  # root-relative
        muco_joint_valid = np.ones((self.muco_joint_num, 3), dtype=np.float32)

        # transform to h36m joint set
        h36m_joint = transform_joint_to_other_db(
            muco_joint, self.muco_joints_name, self.human36_joints_name
        )
        h36m_joint_valid = transform_joint_to_other_db(
            muco_joint_valid, self.muco_joints_name, self.human36_joints_name
        )
