...
class SURREAL(torch.utils.data.Dataset):
    def __init__(self, data_split, args):
        ...
        det_file_path = osp.join(
            self.data_path, f"hrnet_output_on_{self.data_split}set.json"
        )
    ...
    def get_smpl_coord(self, smpl_param):
        ...
        smpl_shape = torch.FloatTensor(shape).view(
            1, -1
    ...
    def __getitem__(self, idx):
        ...
        img_id, img_name, cam_param, bbox, smpl_param, img_shape = (
            data["img_id"],
            data["img_name"],
            data["cam_param"],
            data["bbox"],
        ...
        smpl_coord_cam = (
            smpl_coord_cam
            - smpl_coord_cam[self.smpl_vertex_num + self.smpl_root_joint_idx]
        )
    ...
    def evaluate(self, outs):
        ...
        for n in range(sample_num):
            ...
            eval_result["mesh_error"].append(
                np.sqrt(np.sum((mesh_coord_out - mesh_coord_target) ** 2, 1))
            )  # meter -> milimeter

            # pose error
            eval_result["pose_error"].append(
                np.sqrt(np.sum((joint_coord_out - joint_coord_target) ** 2, 1))
            )  # meter -> milimeter
