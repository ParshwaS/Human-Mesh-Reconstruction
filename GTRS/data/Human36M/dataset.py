...
class Human36M(torch.utils.data.Dataset):
    def __init__(self, mode, args):
        ...
        if self.data_split == "test":
            det_2d_data_path = osp.join(
                cfg.data_dir, dataset_name, "absnet_output_on_testset.json"
            )
            self.datalist_pose2d_det = self.load_pose2d_det(
                det_2d_data_path, skip_img_path
            )
            print(
                "Check lengths of annotation and detection output: ",
                len(self.datalist),
                len(self.datalist_pose2d_det),
            )

        self.graph_Adj, self.graph_L, self.graph_perm, self.graph_perm_reverse = (
            build_coarse_graphs(
                self.mesh_model.face,
                self.joint_num,
                self.skeleton,
                self.flip_pairs,
                levels=9,
    ...
    def get_stat(self):
        ...
        for joint in self.human36_joints_name:
            item = list(
                filter(lambda stat: stat["Joint"] == joint, error_distribution)
            )[0]
    ...
    def load_data(self):
        ...
        for subject in subject_list:
            ...
            with open(
                osp.join(
                    self.annot_path,
                    "Human36M_subject" + str(subject) + "_smpl_param.json",
                ),
                "r",
            ) as f:
        ...
        for aid in db.anns.keys():
            ...
            frame_idx = img["frame_idx"]
            try:
                smpl_param = smpl_params[str(subject)][str(action_idx)][
                    str(subaction_idx)
                ][str(frame_idx)]
            except KeyError:
            ...
            smpl_param["gender"] = (
                "neutral"  # self.subject_genders[subject] # set corresponding gender
            )
    ...
    def get_smpl_coord(self, smpl_param, cam_param):
        ...
        R, t = (
            np.array(cam_param["R"], dtype=np.float32).reshape(3, 3),
        ...
        smpl_mesh_coord, smpl_joint_coord = self.mesh_model.layer[gender](
        ...
        smpl_trans = (
            np.dot(R, smpl_trans[:, None]).reshape(1, 3) + t.reshape(1, 3) / 1000
        )
    ...
    def get_fitting_error(self, h36m_joint, smpl_mesh):
        h36m_joint = (
            h36m_joint - h36m_joint[self.human36_root_joint_idx, None, :]
        )  # root-relative
        ...
        h36m_from_smpl = (
            h36m_from_smpl
            - np.mean(h36m_from_smpl, 0)[None, :]
            + np.mean(h36m_joint, 0)[None, :]
        )
    ...
    def __getitem__(self, idx):
        ...
        img_id, bbox, smpl_param, cam_param, img_shape = (
            data["img_id"],
            data["bbox"].copy(),
            data["smpl_param"].copy(),
            data["cam_param"].copy(),
            data["img_hw"],
        )
        ...
        if not cfg.DATASET.use_gt_input:
            joint_img = self.replace_joint_img(
                idx, img_id, joint_img, tight_bbox, trans
            )
        if flip:
            joint_img = flip_2d_joint(
                joint_img, cfg.MODEL.input_shape[1], self.flip_pairs
            )
    ...
    def replace_joint_img(self, idx, img_id, joint_img, bbox, trans):
        if self.input_joint_name == "coco":
            ...
            if self.data_split == "train":
                ...
                joint_img_coco[:17, :] = synthesize_pose(
                    joint_img_coco[:17, :], area, num_overlap=0
                )
                ...
                for i in range(self.coco_joint_num):
                    joint_img_coco[i, :2] = affine_transform(
                        joint_img_coco[i, :2].copy(), trans
                    )
                return joint_img_coco
            ...
            if self.data_split == "train":
                ...
                for i in range(self.human36_joint_num):
                    joint_img_h36m[i, :2] = affine_transform(
                        joint_img_h36m[i, :2].copy(), trans
                    )
                return joint_img_h36m

    def compute_joint_err(self, pred_joint, target_joint):
        # root align joint
        pred_joint, target_joint = (
            pred_joint - pred_joint[:, :1, :],
            target_joint - target_joint[:, :1, :],
        )

        pred_joint, target_joint = (
            pred_joint.detach().cpu().numpy(),
            target_joint.detach().cpu().numpy(),
        )

        pred_joint, target_joint = (
            pred_joint[:, self.human36_eval_joint, :],
            target_joint[:, self.human36_eval_joint, :],
        )
        joint_mean_error = np.power(
            (np.power((pred_joint - target_joint), 2)).sum(axis=2), 0.5
        ).mean()
    ...
    def compute_both_err(self, pred_mesh, target_mesh, pred_joint, target_joint):
        ...
        pred_mesh, target_mesh = (
            pred_mesh - pred_joint[:, :1, :],
            target_mesh - target_joint[:, :1, :],
        )
        pred_joint, target_joint = (
            pred_joint - pred_joint[:, :1, :],
            target_joint - target_joint[:, :1, :],
        ...
        pred_mesh, target_mesh = (
            pred_mesh.detach().cpu().numpy(),
            target_mesh.detach().cpu().numpy(),
        )
        pred_joint, target_joint = (
            pred_joint.detach().cpu().numpy(),
            target_joint.detach().cpu().numpy(),
        ...
        mesh_mean_error = np.power(
            (np.power((pred_mesh - target_mesh), 2)).sum(axis=2), 0.5
        ).mean()
        joint_mean_error = np.power(
            (np.power((pred_joint - target_joint), 2)).sum(axis=2), 0.5
        ).mean()
    ...
    def evaluate_joint(self, outs):
        ...
        for n in range(sample_num):
            ...
            pose_coord_out, pose_coord_gt = (
                pose_coord_out - pose_coord_out[:1],
                pose_coord_gt - pose_coord_gt[:1],
            )
            # sample eval joitns
            pose_coord_out, pose_coord_gt = (
                pose_coord_out[self.human36_eval_joint, :],
                pose_coord_gt[self.human36_eval_joint, :],
            )
    ...
    def evaluate(self, outs):
        ...
        # eval H36M joints
        pose_error_h36m = np.zeros(
            (sample_num, len(self.human36_eval_joint))
        )  # pose error
        pose_error_action_h36m = [
            [] for _ in range(len(self.action_name))
        ]  # pose error for each sequence
        pose_pa_error_h36m = np.zeros(
            (sample_num, len(self.human36_eval_joint))
        )  # pose error
        pose_pa_error_action_h36m = [
            [] for _ in range(len(self.action_name))
        ]  # pose error for each sequence
        ...
        for n in range(sample_num):
            ...
            joint_coord_out, joint_coord_gt = (
                np.dot(self.joint_regressor_smpl, mesh_coord_out),
                np.dot(self.joint_regressor_smpl, mesh_coord_gt),
            )
            mesh_coord_out = (
                mesh_coord_out
                - joint_coord_out[
                    self.smpl_root_joint_idx : self.smpl_root_joint_idx + 1
                ]
            )
            mesh_coord_gt = (
                mesh_coord_gt
            ...
            action_idx = (
                int(img_name[img_name.find("act") + 4 : img_name.find("act") + 6]) - 2
            )
            ...
            pose_coord_out_h36m = (
                pose_coord_out_h36m - pose_coord_out_h36m[self.human36_root_joint_idx]
            )
