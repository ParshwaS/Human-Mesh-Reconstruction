...
class MSCOCO(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        db = COCO(
            osp.join(
                self.annot_path, "person_keypoints_" + self.data_split + "2017.json"
            )
        )
    ...
    def get_smpl_coord(self, smpl_param):
        ...
        smpl_mesh_coord, smpl_joint_coord = self.mesh_model.layer["neutral"](
    ...
    def get_joints_from_mesh(self, mesh, joint_set_name, cam_param):
        ...
        # projection
        s, t = (
            np.array(cam_param["s"], dtype=np.float32),
            np.array(cam_param["t"], dtype=np.float32).reshape(2),
        )
        joint_coord_img = (joint_coord_cam[:, :2] / 1000) * s + t.reshape(-1, 2)

        joint_coord_img = np.concatenate(
            (joint_coord_img, np.ones((len(joint_coord_img), 1))), axis=1
        )
        return joint_coord_cam, joint_coord_img

    def get_fitting_error(
        self, bbox, coco_from_dataset, coco_from_smpl, coco_joint_valid
    ):
        ...
        coco_from_smpl, _ = j2d_processing(
            coco_from_smpl_xy1, (64, 64), bbox, 0, 0, None
        )
        coco_from_dataset_xy1 = np.concatenate(
            (coco_from_dataset[:, :2], np.ones_like(coco_from_smpl[:, 0:1])), 1
        )
        coco_from_dataset, trans = j2d_processing(
            coco_from_dataset_xy1, (64, 64), bbox, 0, 0, None
        )
        ...
        coco_joint = coco_from_dataset[:, :2][
            np.tile(coco_joint_valid, (1, 2)) == 1
        ].reshape(-1, 2)
        coco_from_smpl = coco_from_smpl[:, :2][
            np.tile(coco_joint_valid, (1, 2)) == 1
        ].reshape(-1, 2)
    ...
    def __getitem__(self, idx):
        ...
        img_shape, bbox, dp_data, smplify_result = (
            data["img_shape"],
            data["bbox"],
            data["dp_data"],
            data["smplify_result"],
        ...
        smpl_param, cam_param = (
            smplify_result["smpl_param"],
            smplify_result["cam_param"],
        )
        ...
        if cfg.MODEL.name == "GTRS_net":
            ...
            error = self.get_fitting_error(
                tight_bbox, data["joint_img"], joint_img_coco[:17], data["joint_valid"]
            )
