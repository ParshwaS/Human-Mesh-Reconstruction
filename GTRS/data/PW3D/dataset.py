...
class PW3D(torch.utils.data.Dataset):
    def __init__(self, data_split, args):
        ...
        self.joint_regressor_human36 = torch.Tensor(
            self.mesh_model.joint_regressor_h36m
        )
        ...
        self.datalist, self.video_indices = (
            self.load_data()
        )  # self.video_indexes: 37 video, and indices of each video
    ...
    def load_data(self):
        ...
        with open(
            osp.join(self.data_path, f"darkpose_3dpw_{self.data_split}set_output.json")
        ) as f:  #
            pose2d_outputs = {}
            data = json.load(f)
            for item in data:
                ...
                pose2d_outputs[annot_id] = {
                    "coco_joints": np.array(item["keypoints"], dtype=np.float32)[:, :3]
                }
        ...
        for aid in db.anns.keys():
            ...
            openpose = self.add_pelvis_and_neck(
                openpose, self.openpose_joints_name, only_pelvis=True
            )
    ...
    def get_coco_from_mesh(self, mesh_coord_cam, cam_param):
        ...
        joint_coord_cam = self.add_pelvis_and_neck(
            joint_coord_cam, self.coco_joints_name
        )
        # projection
    ...
    def __getitem__(self, idx):
        ...
        annot_id, img_id, img_path, img_shape = (
            data["annot_id"],
            data["image_id"],
            data["img_path"],
            data["img_shape"],
        )
        cam_param, bbox, smpl_param = (
            data["cam_param"].copy(),
            data["bbox"].copy(),
    ...
    def compute_joint_err(self, pred_joint, target_joint):
        ...
        pred_joint, target_joint = (
            pred_joint - pred_joint[:, -2:-1, :],
            target_joint - target_joint[:, -2:-1, :],
        )
    ...
    def evaluate(self, outs):
        ...
        for n in range(sample_num):
            ...
            coord_out_cam = (
                coord_out_cam
                - coord_out_cam[self.smpl_vertex_num + self.smpl_root_joint_idx]
            )
            coord_gt_cam = np.concatenate((mesh_coord_gt, joint_coord_gt))
            coord_gt_cam = (
                coord_gt_cam
                - coord_gt_cam[self.smpl_vertex_num + self.smpl_root_joint_idx]
            )
            ...
            pose_coord_out_h36m = np.dot(
                self.mesh_model.joint_regressor_h36m, mesh_coord_out
            )
            pose_coord_out_h36m = (
            ...
            pose_coord_gt_h36m = np.dot(
                self.mesh_model.joint_regressor_h36m, mesh_coord_gt
            )
            pose_coord_gt_h36m = (
            ...
            pred_j3d.append(pose_coord_out_h36m)
            gt_j3d.append(pose_coord_gt_h36m)
