        ...
        for annotation in annotations[0]:
            ...
            for previous_center in previous_centers:
                distance_to_previous = (
                    (person_center[0] - previous_center[0]) ** 2
                    + (person_center[1] - previous_center[1]) ** 2
                ) ** 0.5
            ...
            for i in range(len(annotation["keypoints"]) // 3):
                keypoint = [
                    annotation["keypoints"][i * 3],
                    annotation["keypoints"][i * 3 + 1],
                    2,
                ]
            ...
            for other_annotation in annotations[0]:
                ...
                prepared_other_annotation = {
                    "objpos": [
                        other_annotation["bbox"][0] + other_annotation["bbox"][2] / 2,
                ...
                for i in range(len(other_annotation["keypoints"]) // 3):
                    keypoint = [
                        other_annotation["keypoints"][i * 3],
                        other_annotation["keypoints"][i * 3 + 1],
                        2,
                    ]
            ...
            prepared_annotation["processed_other_annotations"] = (
                prepared_other_annotations
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--net-input-size", type=int, default=368, help="network input size"
    )
    ...
    for annotation in data["annotations"]:
        if annotation["iscrowd"]:
            ...
            crowd_segmentations_per_image_mapping[annotation["image_id"]].append(
    ...
    prepared_annotations = prepare_annotations(
        annotations_per_image_mapping, images_info, args.net_input_size
    )
